...
public class PathWithMaximumGold {
   ...
   /** maxGold = Math.max(maxGold, collectGold(grid, i - 1, j + 1)); // second method
        maxGold = Math.max(maxGold, collectGold(grid, i, j + 1));
        maxGold = Math.max(maxGold, collectGold(grid, i + 1, j + 1)); */
